...
from p01sc01_control_structures.p01m_sales_input import entry, month, fmt_width, sales_list, amount, sales_date, sales
...
def input_month() -> int:
    ...
    ...
    return input_int('month', 12, 1, fmt_width=20)
...
def input_day(year: int, month: int) -> int:
    ...
    ...
    return input_int('day', max_day, 1, fmt_width=20)
...
def get_region_name(region_code: str) -> str:
    ...
    # Complete the task in one return statement
    ...
    return  VALID_REGIONS.get(region_code, 'Unknown Region' )
...
def is_valid_filename_format(filename):
    ...
    """
    ...
    return  len(filename) == len(NAMING_CONVENTION) and filename.startswith('sales_q') and filename.endswith('.csv')
...
def already_imported(filepath_name: Path) -> bool:
    ...
    """
    ...
    imported_file_path = FILEPATH / IMPORTED_FILES

    if not imported_file_path.exists():
        return False
    with imported_file_path.open('r') as file:
...
def add_imported_file(filepath_name: Path):
    ...
    ...
    imported_file_path = FILEPATH / IMPORTED_FILES
    with imported_file_path.open('a') as file:
...
def import_all_sales() -> list:
    ...
    """
    ...
    all_sales_file = FILEPATH / ALL_SALES
    sales_list = []

    if not all_sales_file.exists():
        print(f"File '{ALL_SALES}' does not exist.")
        return  sales_list

    with all_sales_file.open('r', newline='') as csvfile:
        reader = csv.DictReader(csvfile, fieldnames=['amount', 'sales_date', 'region'])
        for row in reader:
            amount = row['amount']
            sales_date = row['sales_date']
            region = row['region']

            if amount and sales_date and region:
                sales_list.append({
                    "amount":amount,
                    "sales_date":sales_date,
                    "region":region
...
def view_sales(sales_list: list) -> bool:
    ...
    if len(sales_list) == 0:  # sales_list could be [] or None
        ...
        for idx, sales in enumerate(sales_list, start=1):
            ...
            # if (isinstance(amount, float) and not has_bad_amount(sales)):
            #     total += amount
            ...
            #     month = 0
            # else:
            #     month = int(sales_date.split("-")[1])  if sales_date else 0

            sales_date = sales.get("sales_date", "")
            month = 0
            if sales_date and len(sales_date) == 10 and sales_date[4] == '-' and sales_date[7] == '-':
                try:
            ...
            region = get_region_name(sales.get("region",""))
...
def add_sales1(sales_list) -> None:
    ...
    print(f"Sales for {entry} is added.\n")
...
def save_all_sales(sales_list, delimiter: str=',') -> None:
    ...
    # Save the converted sales list which now is a list of lists into the file ALL_SALES.
    ...
    headers = ['amount', 'sales_date', 'region']

    data_to_write = [[sales.get('amount', ''),sales.get('sales_date',''),sales.get('region','')]
                     for sales in sales_list]

    data_to_write.insert(0, headers)
    all_sales_path = FILEPATH / ALL_SALES
    with open(FILEPATH / ALL_SALES, mode='w', newline='') as csvfile:
